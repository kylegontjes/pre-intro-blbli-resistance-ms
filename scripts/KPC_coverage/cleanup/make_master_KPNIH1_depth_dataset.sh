...
cat PCMP_H200_summary  | cut -f1 -d, > master_KPNIH1_chromosome_depth_matrix.csv
...
do
    ...
    paste -d',' <(sed '/^$/d' master_KPNIH1_chromosome_depth_matrix.csv) <(sed '/^$/d' temp.csv) > merged.csv
    rm temp.csv
    mv merged.csv master_KPNIH1_chromosome_depth_matrix.csv
done