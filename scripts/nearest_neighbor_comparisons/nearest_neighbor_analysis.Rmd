...
nn_comps_all_vars_of_interest <- lapply(variants,function(variable_name){
  ...
  variant_nn_data <- lapply(isolates_w_variant,FUN=nearest_neighbor_algorithm,phylogenetic_distance = phylo_dist,variant_distance = variant_dist,annotate=T,metadata=metadata,variables_of_interest=c("blbli_dich_num","MVB_dich_num","MVB_num","IR_dich_num","IR_num"), comparison=T,comparison_feature = variable_name) %>% do.call(rbind,.)
