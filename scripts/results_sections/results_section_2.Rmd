...
Clade_MVB_comps <- wilcox.test(MVB_num ~ clade_I,data=df) %>% .[["p.value"]] %>% round(.,digits=4)

CI_IR_MIC_dist <- median_IQR("IR_log_2",df_CI)
CII_IR_MIC_dist <- median_IQR("IR_log_2",df_CII)
Clade_IR_comps <- wilcox.test(IR_num ~ clade_I,data=df) %>% .[["p.value"]] %>% round(.,digits=4)
...
c(24,18,106,248) %>% matrix(ncol=2) %>% fisher.test()
paste0("Clustering, no significant difference but still interesting")
c(5,1,19,17) %>% matrix(ncol=2) %>% fisher.test()
``` 
