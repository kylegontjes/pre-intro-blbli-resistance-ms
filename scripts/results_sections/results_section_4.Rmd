...
mean_rank <- function(group,df){
  df$MVB_ranked <- df[,'MVB_log_2'] %>% rank
  df$IR_ranked <- df[,'IR_log_2'] %>% rank
  present_df <- df %>% subset(get(group)==1)
  absent_df <- df %>% subset(get(group)==0)
  # 
  MVB_present <- paste0(mean(present_df[,'MVB_ranked'])," (",IQR(present_df[,'MVB_ranked']),")")
  IR_present <- paste0(mean(present_df[,'IR_ranked'])," (",IQR(present_df[,'IR_ranked']),")")
  # 
  MVB_absent <- paste0(mean(absent_df[,'MVB_ranked'])," (",IQR(absent_df[,'MVB_ranked']),")")
  IR_absent <- paste0(mean(absent_df[,'IR_ranked'])," (",IQR(absent_df[,'IR_ranked']),")")
  return(data.frame(genotype=group,MVB_present,MVB_absent,IR_present,IR_absent))
}
```
...
df$novel_hits_sig <- ifelse(rowSums(df %>% select(any_of(novel_synchronous))) >0,1,0)
all_canonical <- c("OmpK36_Loss_of_function","OmpK36_promoter_IS","PBP_any","RamA")
df$any_canonical <- ifelse(df %>% select(any_of(all_canonical)) %>% rowSums >0,1,0) 

table(df$novel_hits_sig,df$any_canonical)
table(df$KPNIH1_RS07465_3.2,df$any_canonical)
table(df$KPNIH1_RS18830_2.2,df$any_canonical)
table(df$KPNIH1_RS19680_3.2,df$any_canonical)
table(df$KPNIH1_RS21500_3.2,df$any_canonical)
table(df$KPNIH1_RS26035_2.2,df$any_canonical)
```
...
lapply(continuous_pass$genotype,FUN=mean_rank,df) %>% do.call(rbind,.) %>% favorite_kable
...
```{r} 
convert_tableone_into_df(df,c("OmpK36_L3_mutations","OmpK36GD","OmpK36TD"),strata="clades",outcome_names = c("Clade I","Clade IIA","Clade IIB"),factorVars = c("OmpK36_L3_mutations","OmpK36GD","OmpK36TD"))

# Simple ompK36 status
df$OmpK36_mutations_simple <- ifelse(df$OmpK36_Loss_of_function==1,"Putative function altering variant (PFAV)",
                                     ifelse(df$OmpK36_L3_mutations==1, "Insertion in loop 3","No insertion in loop 3 or PFAV"))

df$OmpK36_mutations_simple <- factor(df$OmpK36_mutations_simple, levels=rev(c("No insertion in loop 3 or PFAV","Insertion in loop 3","Putative function altering variant (PFAV)")))
...
```{r}
df_CI <- df %>% subset(clades == "clade I")
...
df_w_KPC$OmpK36_mutations_simple <- relevel(df_w_KPC$OmpK36_mutations_simple,ref = "No insertion in loop 3 or PFAV") 
df_w_KPC_CI <- df_w_KPC %>% subset(clades == "clade I")
df_w_KPC_CII <- df_w_KPC %>% subset(clades != "clade I")
...
## Validation of hits
```{r}
continuous_pass %>% subset(genotype %in% c("AA018","AA552","FJ223607")) %>% select(locus_tag,product,MVB_num_log_2_diff_increase_prop,MVB_num_log_2_diff_decrease_prop,MVB_num_log_2_diff_median,MVB_num_log_2_diff_range,IR_num_log_2_diff_increase_prop,IR_num_log_2_diff_decrease_prop,IR_num_log_2_diff_median,IR_num_log_2_diff_range,genotype,sig_tests,sig_phenotypes) %>% favorite_kable 
```
