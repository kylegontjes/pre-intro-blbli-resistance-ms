...
novel_continuous <- gwas_table %>% subset(resistance_category == "Modulator" & nn_qc=="Yes" & c(IR_num_log_2_diff_median >1 | MVB_num_log_2_diff_median>1)) %>% .$genotype  %>% subset(!. %in% c("KPNIH1_RS18665","KPNIH1_RS07805",known_hits,"OmpK36-0%",'OmpK36_putative_function_altering','OmpK36GD','OmpK36_loop3_insertion'))
novel_synchronous <-  gwas_table %>% subset(resistance_category != "Modulator" & nn_qc=="Yes") %>% subset(!locus_tag %in% c("KPNIH1_RS18665","KPNIH1_RS07805",known_hits,"OmpK36-0%")) %>% .$genotype %>% subset(!.%in%c('AA018'))
...
BLBLI_exp_panel_CI <-  apply(df_CI %>% select(panels),2,get_marginal_stats,outcome=df_CI$blbli_dich_num)  %>% do.call(rbind,.) %>% as.data.frame%>% mutate(phenotype = c(rep("BLBLI",3)),population = c(rep("CI",3)),panels=panels) 

## Clade II
df_CII <- subset(df,clade_I=="Clade II") 
BLBLI_exp_panel_CII <-  apply(df_CII %>% select(panels),2,get_marginal_stats,outcome=df_CII$blbli_dich_num)  %>% do.call(rbind,.) %>% as.data.frame%>% mutate(phenotype = c(rep("BLBLI",3)),population = c(rep("CII",3)),panels=panels) 
...
BLBLI_panel$population <- factor(BLBLI_panel$population,levels = c("ST258","CI","CII"))  
```

```{r}
A <- ggplot(data=BLBLI_panel,aes(fill=panels,y=sens,x=population)) +  geom_bar(stat = "identity",position='dodge') + theme_bw_me + ylab("Sensitivity") + xlab("Population")  + ylim(0,105) +
     geom_text(aes(label=signif(sens,2)), position=position_dodge(width=0.9), vjust=-0.25)  + panel_scale+ sfig9_format
B <- ggplot(data=BLBLI_panel,aes(fill=panels,y=spec,x=population)) +  geom_bar(stat = "identity",position='dodge')+ theme_bw_me + ylab("Specificity") + xlab("Population") + ylim(0,105)+
     geom_text(aes(label=signif(spec,2)), position=position_dodge(width=0.9), vjust=-0.25) + panel_scale+ sfig9_format
C <- ggplot(data=BLBLI_panel,aes(fill=panels,y=ppv,x=population)) +  geom_bar(stat = "identity",position='dodge')+ theme_bw_me+ ylab("Positive Predictive Value") + xlab("Population") + ylim(0,105)+
     geom_text(aes(label=signif(ppv,2)), position=position_dodge(width=0.9), vjust=-0.25) + panel_scale + sfig9_format
D <- ggplot(data=BLBLI_panel,aes(fill=panels,y=npv,x=population)) +  geom_bar(stat = "identity",position='dodge')+ theme_bw_me+ ylab("Negative Predictive Value") + xlab("Population") + ylim(0,105)+
     geom_text(aes(label=signif(npv,2)), position=position_dodge(width=0.9), vjust=-0.25) + panel_scale + sfig9_format
```

```{r,fig.height=10.625,fig.width=10.625}
plots <- cowplot::plot_grid(A + theme(legend.position = "none"),B+ theme(legend.position = "none"),C+ theme(legend.position = "none"),D+ theme(legend.position = "none"),labels = c("A","B","C","D"),ncol=2,label_size = 18,rel_widths = c(1,1,1,1),label_y = 1,label_x = -0.009)

fig_legend <- get_legend(A )

s_figure_9 <- cowplot::plot_grid(plots,fig_legend,ncol=1,rel_heights=c(1,0.075))
s_figure_9
```

```{r}
ggsave(plot = s_figure_9, filename = './figures/s_figure_9.png', width = 10.625, height = 10.625,limitsize=FALSE,bg='white')
```