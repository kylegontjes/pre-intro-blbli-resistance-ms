...
contig_data$element_symbol <- gsub("KPC-2","<i>bla<sub>KPC-2</sub></i>",contig_data$element_symbol)  %>% gsub("KPC-3","<i>bla<sub>KPC-3</sub></i>",.)  %>% gsub("KPC-5","<i>bla<sub>KPC-5</sub></i>",.) 
...
make_plots <- function(filename,contig_data,blast_data){
  ...
  contig_data$map2_chr <- factor(contig_data$map2_chr,levels = c(isolate_2_contigs %>% arrange(-size) %>% .$contig_name)) 
    ...
    facet_grid(map2_chr~map1_chr,scales = 'free',drop = F)   + xlab(paste0(unique(coords$map1_name)," position (bp)")) + ylab(paste0(unique(coords$map2_name)," position (bp)")) +    geom_point(alpha=.5)  + xlim(0,NA) + ylim(0,NA)+ coord_cartesian(clip = "off") + ggplot2::theme_bw()  + scale_color_manual(values="red",labels="<i>bla<sub>KPC</sub></i>-containing <i>tn4401</i> transposon", name="", guide = guide_legend(nrow=1, title.position = "top", label.position = "right")) + s_figure_6_format
