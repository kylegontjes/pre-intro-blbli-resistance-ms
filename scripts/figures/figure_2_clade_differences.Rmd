...
fig2a <- create_MIC_histogram(df$IR_log_2,df$clade_I,"Imipenem-Relebactam") + ylim(NA,250)   +figure_2_format + clade_colors_scale_v   +MIC_scale + IR_intercepts[[1]] + IR_intercepts[[2]]

MVB_intercepts <-  add_clade_median_values(df, "MVB_log_2", "clade_I")

fig2c <- create_MIC_histogram(df$MVB_log_2,df$clade_I,"Meropenem-Vaborbactam")  + MIC_scale  + ylim(NA,250) +figure_2_format + clade_colors_scale_v + MVB_intercepts[[1]] + MVB_intercepts[[2]]
...
fig2_data <- cowplot::plot_grid(fig2a + theme(legend.position = "none"),fig2b  + theme(legend.position = "none"),"",fig2c + theme(legend.position = "none"),fig2d + theme(legend.position = "none") , fig2e+ theme(legend.position = "none"), ncol=3,rel_widths = c(1,0.45,0.45,1,0.45,0.45),labels = c("A","B","","C","D","E"),label_size = 16)
...
```{r}
ggsave(plot = figure_2, filename = './figures/figure_2.png', width = 9, height = 7.2,bg = 'white')
```