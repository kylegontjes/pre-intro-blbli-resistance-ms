...
```{r}
fig2a <- create_MIC_histogram(df$MVB_log_2,df$clade_I,"Meropenem-Vaborbactam")  + scale_x_continuous(breaks=c(-2,-1,0,1,2,3,4,5),
        labels=c("≤0.25","0.5", "1","2","4","8","16","≥32"))     + ylim(NA,250) +figure_2_format + clade_colors_scale_v
   
fig2c <- create_MIC_histogram(df$IR_log_2,df$clade_I,"Imipenem-Relebactam")+ scale_x_continuous(breaks=c(-2,-1,0,1,2,3,4,5),
        labels=c("≤0.25","0.5","1","2","4","8","16","≥32"))    + ylim(NA,250)   +figure_2_format + clade_colors_scale_v
```
...
blbli_clade_phylo_prop$phenotype_cat <- factor(blbli_clade_phylo_prop$phenotype_cat,levels = c("Singleton",paste0("Cluster ",1:6)))


fig2e <- MIC_prop_hist_count(blbli_clade_phylo_prop) + cluster_scale + figure_2_format  
```

# Figure Compilation
```{r,message=F,echo=F,error=F,warning=F,fig.width=20,fig.height=12.5}
fig2_data <- cowplot::plot_grid(fig2a + theme(legend.position = "none"),fig2b  + theme(legend.position = "none"),"",fig2c + theme(legend.position = "none"),fig2d + theme(legend.position = "none") , fig2e+ theme(legend.position = "none"), ncol=3,rel_widths = c(1,0.45,0.45,1,0.45,0.45),labels = c("A","B","","C","D","E"),label_size = 22)
...
# Figure
```{r,message=F,echo=F,error=F,warning=F,fig.width=12.5,fig.height=9.25}
figure_2
```

```{r}
ggsave(plot = figure_2, filename = './figures/figure_2.png', width = 12.5, height = 9.25)
```