...
B_w_tree <- gheatmap(B_1,df %>% select(blbli_asr_cluster_renamed) %>% `colnames<-`("BL/BLI clustering"),width=0.05,colnames_position = 'top',colnames_angle = 90, color=NULL ,colnames_offset_y = 0.25, hjust = 0)  + ylim(NA, 560) + theme(legend.position = 'bottom', legend.text = element_markdown()) + cluster_scale_2_col_key 
```
...
ggsave(plot = s_figure_12, filename = './figures/s_figure_12_genotype_clustering.png', width = 11, height = 6.5, limitsize=FALSE, bg='white', dpi=900)
```