---
title: "Supplemental Figure 12: Differential potential for emergence and spread of β-lactam/β-lactamase inhibitor resistance-associated genotypes "
...
```{r}
packages <- c("tidyverse","cowplot","ggtree","ape","phytools","gridExtra","kableExtra","colorRamp2","RColorBrewer","ComplexHeatmap",'phyloAMR','cowplot')
...
source("./scripts/figures/s_figure_11_functions.R")
...
row_names <- variables %>% gsub("_1.2|_1|_2.2|_2|_3|_3.2","",.)  %>% recode_genotypes(.)
...
},border_gp = gpar(col = "black", lty = 1), rect_gp = gpar(col = "white", lwd = 1),bottom_annotation = clade_labels,row_names_gp =  gpar(fontsize = 11),column_names_gp =  gpar(fontsize = 13),heatmap_legend_param = heatmap_legend_params, 
width = unit(10, "cm"),
...
B_w_tree <- gheatmap(B_1,df %>% select(blbli_asr_cluster_renamed) %>% `colnames<-`("BL/BLI clustering"),width=0.05,colnames_position = 'top',colnames_angle = 90, color=NULL ,colnames_offset_y = 0.25, hjust = 0,font.size=3.75)  + ylim(NA, 535) + theme(legend.position = 'bottom', legend.text = element_markdown(),legend.box.margin = margin(t = -25)) + cluster_scale_2_col_key 
```

# Supplemental figure

```{r, fig.height=6.285714, fig.width=11} 
s_figure_12.A <- ggdraw() + draw_image(tmpfile)
unlink(tmpfile) 

s_figure_12 <- cowplot::plot_grid(s_figure_12.A, B_w_tree, labels = "AUTO", ncol=2, label_size = 20, rel_widths = c(1,0.65))
...
ggsave(plot = s_figure_12, filename = './figures/s_figure_12_genotype_clustering.png', width = 11, height = 6.285714, limitsize=FALSE, bg='white', dpi=900)
```