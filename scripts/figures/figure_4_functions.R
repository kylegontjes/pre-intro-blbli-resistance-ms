...
MIC_ompk36_prop_hist <- function(dataset,title,colors){
  ...
  histogram <-  ggplot(data=dataset,aes(x=phenotype_cat,fill=OmpK36_mutations,y=freq))+geom_bar(position="stack",stat="identity")  +ylab("Proportion of Isolates") +xlab("Resistance Profile") + labs(fill="ompK36 Profile") + scale_fill_manual(values = colors)    + ggtitle(paste0(title)) + theme(legend.position = "bottom",
