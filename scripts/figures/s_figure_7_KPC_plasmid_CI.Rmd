...
A <-  gheatmap(p.8,df_CI_relevant_plasmids  %>% select(AA018_KPC,AA018_non_KPC,AA552_KPC,AA552_non_KPC,Other_KPC_cluster) %>% mutate_all(as.factor) %>% `colnames<-`(c("AA018 KPC plasmid","AA018 non-KPC plasmid","AA552 KPC plasmid","AA552 non-KPC plasmid","Other KPC plasmids")), colnames_position = "top",colnames_angle=90, colnames_offset_y = 0.25, hjust = 0, color = NA, font.size = 6.5, width = 0.375,offset =.0000875) + feature_scale_v   + ylim(NA,230) + theme(legend.position = 'bottom') + consistent_theme_sfigure_5
...
C <-  ggplot(data=blbli_plasmid_prop,aes(x=clustering_simple,fill=AA552_like,y=n))+geom_bar(position="stack",stat="identity",width = .75)  +ylab("Isolates") +xlab("Phylogenetics of resistance")   + theme_bw_me + s_figure_5_descriptive_plot_format  +ylim(NA,115) + AA552_fill + theme(legend.title = element_markdown(),legend.text=element_markdown()) + scale_y_continuous(breaks = scales::pretty_breaks(n=5))
C
```
...
ggsave(plot = s_figure_5, filename = './figures/s_figure_7_plasmids_clade_I.png', width = 19.75, height = 9.875, limitsize=FALSE, bg='white',  dpi=900)
```