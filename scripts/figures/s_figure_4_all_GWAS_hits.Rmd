...
s_figure_4.A_tb$locus_tag <- recode_genotypes(s_figure_4.A_tb$locus_tag)
s_figure_4.A_tb$locus_tag <- ifelse(s_figure_4.A_tb$locus_tag =="KPNIH1_RS18665","ompK36 PFAV",s_figure_4.A_tb$locus_tag)
s_figure_4.A_tb$gene <- ifelse(grepl(pattern = 'ompK36|OmpK36',s_figure_4.A_tb$locus_tag) | s_figure_4.A_tb$locus_tag =='KPNIH1_RS18665',"ompK36",s_figure_4.A_tb$gene) %>% {ifelse(is.na(.),"",.)}
...
# Add PFAV call 
s_figure_4.A_tb$sig_models_simple <- gsub("High-risk variants","PFAV",s_figure_4.A_tb$sig_models_simple)

# Recode PFAV at first mention
idx <- which(grepl("PFAV", s_figure_4.A_tb$sig_models_simple))[1]
s_figure_4.A_tb$sig_models_simple[idx] <- gsub(
  "PFAV",
  "Putative function-altering variants<br>(PFAVs)",
  s_figure_4.A_tb$sig_models_simple[idx])
...
s_figure_4.A_tbl <- s_figure_4.A_tb %>% select(locus_tag,gene,product,sig_tests,sig_models_simple,sig_phenotypes,nn_qc) %>% `colnames<-`(c('Significant hit','Gene', 'Product', 'Significant test(s)','Significant model(s)', 'Phenotype(s)','Nearest neighbor criterion'))
...
webshot2::webshot("tab_kbl.html", "tab_kbl.png", vwidth = 3900, vheight = 700) 
```
...
sig_hits_name <- recode_genotypes(colnames(s_figure_4.df_mat))
...
```{r ,fig.height=18.5,fig.width=22}
...
# Save figure 
```{r}
ggsave(plot = s_figure_4, filename = './figures/s_figure_4.png', width = 22, height = 18.5,limitsize=FALSE,bg='white',dpi=900)
```
