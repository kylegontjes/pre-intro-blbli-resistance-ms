...
```{r}
blast_lr <- read.delim("./data/tn4401_blast_hybrid/tn4401_blast_hybrid_master.txt",sep = ' ')
...
blast_lr$element_symbol <- blast_lr$sseqid 
...
```{r}
kpc_contigs <- blast_lr %>% subset(element_symbol  =='Tn4401b-1') %>% select(isolate_no,contig,element_symbol) %>% distinct
...
contig_data$contig_name <- paste0(contig_data$contig,"-",contig_data$primary_cluster_id,"-",contig_data$mash_nearest_neighbor,"-",contig_data$element_symbol,"-",contig_data$size,"bp") %>% gsub("---","",.) %>% gsub("NA","",.) %>% trimws(.,"right","-") %>% gsub("--","-",.)
```
...
ggsave("../../figures/s_figure_8.png",plot=tn4401_hopping,width = 17,height = 13)
```
