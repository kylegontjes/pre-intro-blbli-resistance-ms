---
title: "Supplemental Figure 9 Differential potential for emergence and spread of β-lactam/β-lactamase inhibitor resistance-associated genotypes "
...
source("./scripts/figures/s_figure_9_functions.R")
...
# Heatmap
A_heatmap <- ComplexHeatmap::Heatmap(prop_matrix,show_heatmap_legend = F,show_row_names = T,col = c("white","red"),cluster_columns = T,cluster_rows=T,name = "Proportion",column_labels = column_names,row_labels = row_names, cell_fun = function(j, i, x, y, width, height, fill) {
    grid.text(sprintf("%.3s", ct_matrix[i, j]), x, y, gp = gpar(fontsize = 8))
},border_gp = gpar(col = "black", lty = 1), rect_gp = gpar(col = "white", lwd = 1),bottom_annotation = clade_labels,row_names_gp =  gpar(fontsize = 11),column_names_gp =  gpar(fontsize = 11))


fig_heatmap_image <- grid.grabExpr(draw(A_heatmap))
fig_legend_image <- grid.grabExpr(draw(fig_legend))

A <- plot_grid(fig_heatmap_image,fig_legend_image,ncol=1,axis="l", align="hv",rel_heights = c(1,.2))   
```
...
ompK36_name = "ompK36 PFAV mutant"
PBP_name = "PBP-2 non-synonymous mutant"
RamA_name = "ramA PFAV mutant" 
...
B <- st258 + 
  ...
  scale_fill_manual(name="Synchronous clustering",values = setNames(
    c("red", "#91796A", "#97C466", "#9451AD","#B9964B"), genotype_ordering
    ...
    c(22,21,23,24,25),genotype_ordering
  ),breaks=genotype_ordering,guide = guide_legend(ncol=1,title.position = "top", label.position = "right")) + 
  scale_size_manual(name="Synchronous clustering",values = setNames(
    c(5,3.25,3,2,3),genotype_ordering),breaks=genotype_ordering,guide = guide_legend(ncol=1,title.position = "top", label.position = "right"))  + 
  scale_alpha(guide = 'none') + theme(legend.position = 'bottom')
rownames(df) <- df$isolate_no

B_1 <- B + ggnewscale::new_scale_fill()

B_w_tree <- gheatmap(B_1,df %>% select(blbli_asr_cluster_renamed) %>% `colnames<-`("BL/BLI Clustering"),width=0.05,colnames_position = 'top',colnames_angle = 90, color=NULL ,colnames_offset_y = 0.25, hjust = 0) + cluster_scale_2_col + ylim(NA,525) + theme(legend.position = 'bottom')
```

# Cowplot supplemental figure
```{r, fig.height=7.5, fig.width=12}
A_grob <- ggplotify::as.ggplot(A) + theme(plot.margin = unit(c(0, 0.5, 0, 0.45), "inches"))
s_figure_10 <- cowplot::plot_grid(A_grob,B_w_tree,labels = "AUTO",ncol=2,label_size = 20,rel_widths = c(1,0.65))
s_figure_10
``` 

```{r,message=F,echo=F,error=F,warning=F,fig.width=20,fig.height=12.5} 
ggsave(plot = s_figure_10, filename = './figures/s_figure_9.png', width = 12, height = 7.5,limitsize=FALSE,bg='white')
```