...
genotype_count <- function(dataframe,annots){
  ...
  results_df <- cbind.data.frame(raw_rownames=variants,genotype_ct=genotype_ct,genotype_freq=genotype_freq,genotype_freq_under_1=genotype_freq_under_1)
  return(results_df)
  }
...
variant_freq_histogram <- function(annots_df,bins=NULL){
  figure <- ggplot(data=annots_df) + geom_histogram(aes(x=genotype_freq,fill=var_cat),bins=bins) + theme_bw_me + xlab("Proportion of isolates with variant") + ylab("Number of variants") + theme(legend.position = "bottom") + variant_type_scale
  return(figure)
}
...
s_figure_10 <- s_figure_10_main +  annotation_custom(ggplotGrob(s_figure_10_inlet), xmin = 0.4, xmax = 1, ymin = 1250, ymax = 4000)
s_figure_10 
```

```{r,message=F,echo=F,error=F,warning=F,fig.width=5.5,fig.height=4.5} 
ggsave(plot = s_figure_10, filename = './figures/s_figure_10.png', width = 5, height = 4,limitsize=FALSE)
```
