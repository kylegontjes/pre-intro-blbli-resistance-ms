...
s_figure_12_main <- variant_freq_histogram(all_variants_annots,bins=100)   + s_figure_12_format
s_figure_12_inlet <- variant_freq_histogram(all_variants_annots %>% subset(genotype_freq<=0.05),bins=30) + theme(legend.position = "none") + xlab("") + ylab("")

s_figure_12 <- s_figure_12_main +  annotation_custom(ggplotGrob(s_figure_12_inlet), xmin = 0.4, xmax = 1, ymin = 1250, ymax = 4000)
s_figure_12 
```

```{r,message=F,echo=F,error=F,warning=F,fig.width=6,fig.height=5} 
ggsave(plot = s_figure_12, filename = './figures/s_figure_10.png', width = 6, height = 5,limitsize=FALSE)
```
