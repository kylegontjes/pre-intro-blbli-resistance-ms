...
# Relevant alleles
  ...
  s_figure_3_A <-  gheatmap(p9.1,df %>% select(any_of(carbapenem)) %>% mutate_all(as.factor) %>% `colnames<-`(recode_genotypes(colnames(.))), colnames_position = "top",colnames_angle=90, colnames_offset_y = 0.25, hjust = 0, color = NA, font.size = 5.5, width = 0.7,offset =.0000875) + genotype_scale_v  + consistent_theme_sfigure_3 + ylim(NA,665)
```
...
gwas_nons_freq$`Significant Hit` <- recode_genotypes(gwas_nons_freq$locus_tag) 
