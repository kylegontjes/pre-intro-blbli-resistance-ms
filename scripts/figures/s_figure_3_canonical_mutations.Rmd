...
KPC_data$KPC_allele <- ifelse(grepl("KPC-3",KPC_data$sseqid) ==T,"blaKPC-3",
                                ...
                                ifelse(grepl("KPC-10",KPC_data$sseqid) ==T,"KPC-10","No blaKPC"))))
df <- left_join(df,KPC_data %>% select(isolate_no,KPC_allele))
df$KPC_allele <- ifelse(is.na(df$KPC_allele),"No blaKPC",df$KPC_allele)
df$`KPC-10` <- ifelse(df$KPC_allele=="blaKPC-10",1,0)

## Edit KPC type with KPC-10 data
df$KPC_allele <- ifelse(df$`KPC-10`==1,"blaKPC-10",df$KPC_allele)
```
...
rownames(df) <- df$isolate_no 
  ...
  p.7 <-  gheatmap(p.6,df %>% select(MVB_log_2,IR_log_2) %>%   mutate_all(as.factor)  %>%`colnames<-`(c("Meropenem-vaborbactam MIC","Imipenem-relebactam MIC")), colnames_position = "top",colnames_angle=90, colnames_offset_y = 0.25, hjust = 0, color = NA, font.size = 5.5, width = 0.1,offset =.0000375) + Log2_scale + consistent_theme_sfigure_3
...
# KPC allele
  ...
  p9 <-  gheatmap(p8.1,df %>% select(KPC_allele) %>% mutate_all(as.factor)  %>% `colnames<-`("blaKPC allele"), colnames_position = "top",colnames_angle=90, colnames_offset_y = 0.25, hjust = 0, color = NA, font.size = 5.5, width = 0.05,offset =.000075)  +kpc_scale + consistent_theme_sfigure_3 + ylim(NA,480)
0
# relevant alleles
  p9.1 <- p9 + new_scale_fill()
  s_figure_3_A <-  gheatmap(p9.1,df %>% select(any_of(carbapenem)) %>% mutate_all(as.factor) %>% `colnames<-`(recode(colnames(.,),'OmpK35-25'='Truncation in ompK35 porin at 25%','OmpK36_c25t'='25 cytosine-to-thymine transition in ompK36','OmpK36_L3_mutations'='Loop 3 insertion in OmpK36','OmpK36GD'='GD loop 3 insertion in OmpK36','OmpK36TD'='TD loop 3 insertion in OmpK36','OmpK36_truncation_kleborate'='Truncation in ompK36', 'OmpK36_non_syn'='Non-synonymous mutation in ompK36','OmpK36_putative_function_altering'='PFAV in ompK36','OmpK36_intergenic'='Mutation in 5\' intergenic region of ompK36','OmpK36_promoter_IS'='Insertion sequence near ompK36 promoter',"AcrAB_TolC_any" = 'acrAB-tolC efflux pump mutant','RamR'='PFAV in ramR efflux pump regulator',"RamA"='PFAV in ramA efflux pump activator','PBP_any'='Non-synonymous mutations in penicillin-binding-proteins','PBP2'='Penicillin-binding protein-2 mutant',"PBP4"='Penicillin-binding protein-4 mutant')), colnames_position = "top",colnames_angle=90, colnames_offset_y = 0.25, hjust = 0, color = NA, font.size = 5.5, width = 0.7,offset =.0000875) + feature_scale_v  + consistent_theme_sfigure_3 + ylim(NA,665)
```

# FB - Frequency in resistant isolates
...
# FC - Nearest neighbor algorithm data
```{r,fig.height=7.5,fig.width=10}
...
# Merged figure
```{r,fig.height=12.1667,fig.width=18.25}
s_figure_3BC <- plot_grid(s_figure_3_B,s_figure_3_C ,nrow = 2,labels = c("B","C"),label_size = 22,align = "hv",label_x = -0.0355)

s_figure_3 <- plot_grid(s_figure_3_A,s_figure_3BC,labels = c("A",""),label_size = 22,ncol = 2,rel_heights = c(1,1),rel_widths = c(1,0.6)) 
...
ggsave(plot = s_figure_3, filename = './figures/s_figure_3.png', height = 12.1667, width = 18.25, limitsize=FALSE, bg='white', dpi=900)
```
