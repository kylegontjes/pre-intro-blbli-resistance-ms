...
MVB_CI <- KPC_copy_number_ompk_hist(df_CI %>% subset( KPNIH1_RS28775_chr_median_norm!=0),y="MVB_log_2",x="KPNIH1_RS28775_chr_median_norm",color = "OmpK36_mutations_simple",title = "",colors = ompK36_colors) + ylab("Log<sub>2</sub> MVB MIC") + xlab("Log<sub>2</sub> bla<sub>KPC</sub></i> copy number") + ylim(-2.25,6) + xlim(-2.75,5) + figure_4_format + ompK36_color_scale
IR_CI <- KPC_copy_number_ompk_hist(df_CI %>% subset( KPNIH1_RS28775_chr_median_norm!=0),y="IR_log_2",x="KPNIH1_RS28775_chr_median_norm",color = "OmpK36_mutations_simple",title = "",colors = ompK36_colors) + ylab("Log<sub>2</sub> IR MIC") + xlab("Log<sub>2</sub> bla<sub>KPC</sub></i> copy number") + ylim(-2.25,6)+ xlim(-2.75,5) + figure_4_format + ompK36_color_scale

MVB_CII <- KPC_copy_number_ompk_hist(df_CII %>% subset( KPNIH1_RS28775_chr_median_norm!=0),y="MVB_log_2",x="KPNIH1_RS28775_chr_median_norm",color = "OmpK36_mutations_simple",title = "",colors = ompK36_colors) + ylab("Log<sub>2</sub> MVB MIC") + xlab("Log<sub>2</sub> bla<sub>KPC</sub></i> copy number") + ylim(-2.25,6)+ xlim(-2.75,5) + figure_4_format + ompK36_color_scale
IR_CII <- KPC_copy_number_ompk_hist(df_CII %>% subset( KPNIH1_RS28775_chr_median_norm!=0),y="IR_log_2",x="KPNIH1_RS28775_chr_median_norm",color = "OmpK36_mutations_simple",title = "",colors = ompK36_colors) + ylab("Log<sub>2</sub> IR MIC") + xlab("Log<sub>2</sub> bla<sub>KPC</sub></i> copy number") + ylim(-2.25,6)+ xlim(-2.75,5) + figure_4_format + ompK36_color_scale
```
...
s_figure_8 <- cowplot::plot_grid(s_fig_plots,legend,rel_heights = c(1,0.075),rel_widths=c(1,0.75),ncol=1)
s_figure_8
```
 
```{r,message=F,echo=F,error=F,warning=F,fig.width=20,fig.height=12.5} 
ggsave(plot = s_figure_8, filename = './figures/s_figure_8.png', width =7.5, height = 7.5,limitsize=FALSE,bg='white',dpi=800)
```
