---
title: "Supplemental Figure 1 - Cross-resistance profile of 412 carbapenem-resistant Klebsiella pneumoniae ST258 isolates"
...
## Heatmap
co_occurrence_heatmap <- ComplexHeatmap::pheatmap(co_occurrence_percent %>% as.matrix,border_color = "black",name ="Cross-Resistance",legend=F,fontsize = 20,
    row_names_max_width = max_text_width(rownames(co_occurrence_merged), gp = gpar(fontsize = 26)),
    column_names_max_height = max_text_width(rownames(co_occurrence_merged), gp = gpar(fontsize = 26)),
...
fig_legend <- Legend(at = c(0, 25, 50, 75, 100),labels = c("0%", "25%", "50%", "75%", "100%"),title = "Cross-Resistance",direction = "horizontal",col_fun = colorRamp2(breaks = one_hundred_breaks,colors = colorRampPalette(rev(brewer.pal(n = 7, name =  "RdYlBu")))(100)), title_gp = gpar(fontsize = 26),labels_gp=gpar(fontsize=24),legend_width = unit(18, "cm"))

# Print heatmap
fig_heatmap_image <- grid.grabExpr(draw(co_occurrence_heatmap))
fig_legend_image <- grid.grabExpr(draw(fig_legend))

s_figure_1 <-plot_grid(fig_heatmap_image,NULL,fig_legend_image,ncol=1,axis="l", align="hv",rel_heights = c(1,-.085,.25))  
```

# Print & Save
```{r,message=F,error=F,warning=F,fig.width=16.25,fig.height=8.5}
s_figure_1

ggsave(plot = s_figure_1, filename = './figures/s_figure_1.png', width = 16.25, height = 8.5)
```