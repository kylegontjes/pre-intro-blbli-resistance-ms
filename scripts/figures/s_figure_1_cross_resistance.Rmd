...
## Heatmap
co_occurrence_heatmap <- ComplexHeatmap::pheatmap(co_occurrence_percent %>% as.matrix,border_color = "black",name ="Cross-Resistance",legend=F,fontsize = 18,
    row_names_max_width = max_text_width(rownames(co_occurrence_merged), gp = gpar(fontsize = 24)),
    column_names_max_height = max_text_width(rownames(co_occurrence_merged), gp = gpar(fontsize = 24)),
...
fig_legend <- Legend(at = c(0, 25, 50, 75, 100),labels = c("0%", "25%", "50%", "75%", "100%"),title = "Cross-Resistance",direction = "horizontal",col_fun = colorRamp2(breaks = one_hundred_breaks,colors = colorRampPalette(rev(brewer.pal(n = 7, name =  "RdYlBu")))(100)), title_gp = gpar(fontsize = 24),labels_gp=gpar(fontsize=22),legend_width = unit(15, "cm"),title_position = 'topcenter')

# Print heatmap
fig_heatmap_image <- grid.grabExpr(draw(co_occurrence_heatmap))
fig_legend_image <- grid.grabExpr(draw(fig_legend))

s_figure_1 <-plot_grid(fig_heatmap_image,fig_legend_image,ncol=1,axis="l", align="hv",rel_heights = c(1,.125))  
```

# Print & Save
```{r,message=F,error=F,warning=F,fig.width=14.5,fig.height=6.75}
s_figure_1
```

```{r}
ggsave(plot = s_figure_1, filename = './figures/s_figure_1.png', width = 14.5, height = 6.75, bg = "white")
```