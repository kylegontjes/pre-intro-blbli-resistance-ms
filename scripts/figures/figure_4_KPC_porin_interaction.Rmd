...
# Figures
## Imipenem-relebactam
fig4a <- create_MIC_histogram(df$IR_log_2,df$OmpK36_mutations_simple,"Imipenem-relebactam")+ MIC_IR_x_scale  + labs(fill="ompK36 Status")  + theme_bw_me + ylim(NA,250) + figure_4_format_wo_element_markdown + ompK36_scale  + IR_intercepts[[1]] + IR_intercepts[[2]] + IR_intercepts[[3]]  

## Meropenem-vaborbactam
fig4c <- create_MIC_histogram(df$MVB_log_2,df$OmpK36_mutations_simple,"Meropenem-vaborbactam") + MIC_MVB_x_scale + labs(fill="ompK36 Status") + theme_bw_me + ylim(NA,250) + figure_4_format_wo_element_markdown + ompK36_scale  + MVB_intercepts[[1]] + MVB_intercepts[[2]] + MVB_intercepts[[3]]
```
...
# Imipenem-relebactam
fig4e <- KPC_copy_number_ompk_hist_facet(df_w_KPC, y="IR_log_2", x="KPNIH1_RS28775_chr_median_norm", color = "OmpK36_mutations_simple_strata", title="", colors = ompK36_colors) + ylab("Log<sub>2</sub> Imipenem-relebactam MIC") + xlab("Log<sub>2</sub> <i> bla</i><sub>KPC</sub> copy number") + figure_4_format + ompK36_color_scale  

# Meropenem-vaborbactam
fig4f <- KPC_copy_number_ompk_hist_facet(df_w_KPC, y="MVB_log_2", x="KPNIH1_RS28775_chr_median_norm", color = "OmpK36_mutations_simple_strata", title="", colors = ompK36_colors) + ylab("Log<sub>2</sub> Meropenem-vaborbactam MIC") + xlab("Log<sub>2</sub> <i> bla</i><sub>KPC</sub> copy number") + figure_4_format + ompK36_color_scale  
```
...
fig4.data.all <- cowplot::plot_grid(fig4.data, fig4.data.2, ncol=1, rel_heights = c(1,.8))
...
# Final figure
figure_4 <-  cowplot::plot_grid(fig4.data.all,figure_4_legends,rel_heights = c(1,0.105),rel_widths=c(1,0.75),ncol=1) + theme(plot.margin = unit(c(t=0, 0, b=-1,0 ), "cm"))
...
ggsave(plot = figure_4, filename = './figures/figure_4.png', width = 9.25, height = 9.25, limitsize=FALSE, bg='white', dpi=900)
```