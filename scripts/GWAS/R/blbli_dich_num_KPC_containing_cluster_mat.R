...
#Genotypes
geno <- readRDS(file=paste0('./data/GWAS/matrices/','KPC_containing_cluster_mat','.RDS')) %>% subset(rownames(.) %in% rownames(df)) %>% .[match(tr$tip.label,rownames(.)),] %>% as.data.frame %>% select_if(.,colSums(.) > 1 & colSums(.) < nrow(.) - 2) %>% as.matrix
...
hogwash(
  ...
  tree = tr,
  fdr = 0.10,
  file_name = paste0('blbli_dich_num','_','KPC_containing_cluster_mat'),
